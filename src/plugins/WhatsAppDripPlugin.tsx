...
const DEFAULT_CONFIG: WhatsAppDripConfig = {
  messages: [
    { id: '1', content: 'Welcome! Thank you for your interest. How can we help you today?', delay: 0, active: true },
    { id: '2', content: 'Just checking in. Do you have any questions about our services?', delay: 24, active: true },
    { id: '3', content: 'Final follow-up: Would you like to schedule a demo or consultation?', delay: 72, active: true }
  ],
...
const WhatsAppDripPlugin: React.FC = () => {
  ...
  const [newMessage, setNewMessage] = useState<Omit<Message, 'id'>>({ content: '', delay: 24, active: true });
  ...
  const savePlugin = useMutation({
      ...
      if (plugin?.id) {
        ...
        const { data, error } = await supabase
          ...
          .insert({
            ...
            description: 'Automated WhatsApp message drip campaign for leads and customers',
            status: pluginConfig.status,
            xp: 0,
            roi: 0,
            category: 'messaging',
    ...
    onSuccess: () => {
      ...
      toast({
        title: 'Plugin saved',
        description: 'WhatsApp Drip Campaign plugin has been saved successfully',
      });
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || 'An error occurred while saving the plugin',
        variant: 'destructive',
      });
    }
  });
  ...
  useEffect(() => {
    if (plugin?.metadata) {
      try {
        const pluginConfig = typeof plugin.metadata === 'string' 
  ...
  const addMessage = () => {
    ...
    const newId = (Math.max(0, ...config.messages.map(m => parseInt(m.id))) + 1).toString();
  ...
  const updateTwilioConfig = (field: keyof WhatsAppDripConfig, value: string) => {
    setConfig(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <p className="text-muted-foreground mb-8">Configure automated WhatsApp message sequences for your leads and customers</p>
      
      {isLoading ? (
        <div className="py-12 text-center">
          <Loader className="h-8 w-8 animate-spin mx-auto" />
          <p className="mt-2 text-muted-foreground">Loading plugin configuration...</p>
        </div>
      ) : (
              ...
              <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <CardDescription>Configure your drip campaign message sequence</CardDescription>
                  </div>
                    ...
                    {config.messages.length === 0 ? (
                      ...
                      config.messages.map((message) => (
                        <TableRow key={message.id}>
                          <TableCell className={!message.active ? "text-muted-foreground" : ""}>
                            {message.content}
                            {message.delay === 0 && (
                              <Badge variant="outline" className="ml-2">Initial</Badge>
                            )}
                          </TableCell>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-10 gap-4">
                    ...
                    <div className="md:col-span-2">
                      <div className="flex items-center space-x-2">
                        <Label htmlFor="delay" className="whitespace-nowrap">Delay (h):</Label>
                        <Input 
                          id="delay" 
                          type="number" 
                          min="0"
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input 
                    ...
                    onChange={(e) => updateTwilioConfig('twilioAccountSid', e.target.value)}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input 
                    ...
                    onChange={(e) => updateTwilioConfig('twilioAuthToken', e.target.value)}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input 
                    ...
                    onChange={(e) => updateTwilioConfig('twilioPhoneNumber', e.target.value)}
                  />
                </div>
                
                <div className="mt-6 p-3 bg-muted rounded-md">
                  <p className="text-sm text-muted-foreground">
                    Note: In development mode, messages will be logged but not actually sent.
                    To enable real message sending, configure valid Twilio credentials.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="plugin-status" className="text-base font-medium">Enable Plugin</Label>
                    <p className="text-sm text-muted-foreground">Activate the WhatsApp drip campaign</p>
                  </div>
                  <Switch 
                    id="plugin-status" 
                ...
                {!isTwilioConfigured && config.status === 'active' && (
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-900/30 text-yellow-800 dark:text-yellow-500 p-3 rounded-md text-sm">
                    Please complete Twilio configuration before activating the plugin.
                  </div>
                )}
                ...
                <div>
                  <Label className="text-base font-medium mb-2 block">Target Audiences</Label>
                  <div className="grid grid-cols-2 gap-2">
                        ...
                        onCheckedChange={(checked) => {
                          setConfig(prev => ({
                            ...
                            enabledFor: checked 
                              ...
                              : prev.enabledFor.filter(t => t !== 'prospects')
                          }));
                          setIsConfigChanged(true);
                        }}
                      />
                      <label htmlFor="prospects" className="text-sm">Prospects</label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox 
                        id="customers"
                        ...
                        onCheckedChange={(checked) => {
                          setConfig(prev => ({
                            ...
                            enabledFor: checked 
                              ...
                              : prev.enabledFor.filter(t => t !== 'customers')
                          }));
                          setIsConfigChanged(true);
                        }}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor="churned" className="text-sm">Churned</label>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <Label className="text-base font-medium mb-2 block">History</Label>
                  {plugin?.created_at && (
                    <div className="text-sm">
                      <p className="flex justify-between">
                        <span className="text-muted-foreground">Created:</span> 
                        <span>{format(new Date(plugin.created_at), 'PPP')}</span>
                      </p>
                      {plugin.updated_at && (
                        <p className="flex justify-between mt-1">
                          <span className="text-muted-foreground">Last updated:</span>
                          <span>{format(new Date(plugin.updated_at), 'PPP')}</span>
                        </p>
                      )}
                    </div>
                  )}
