 ...
 */
export function useOptimizedSystemLogsData(initialFilters: Partial<LogFilters> = {}) {
  const [filters, setFilters] = useState<LogFilters>({
    ...initialFilters
  });

  // Fetch logs using React Query for caching and automatic refetching
  const { data: logs = [], isLoading, error: fetchError, refetch } = useQuery({
  ...
  const applyFilters = useCallback((data: SystemLog[], currentFilters: LogFilters) => {
    let result = [...data];
    ...
    if (currentFilters.tenant_id) {
      result = result.filter(log => log.tenant_id === currentFilters.tenant_id);
    }
    
    // Filter by date range - handle all date field aliases
    const fromDate = currentFilters.fromDate || currentFilters.startDate || currentFilters.dateFrom;
    if (fromDate) {
      const dateFrom = new Date(fromDate);
      result = result.filter(log => new Date(log.created_at) >= dateFrom);
    }
    
    const toDate = currentFilters.toDate || currentFilters.endDate || currentFilters.dateTo;
    if (toDate) {
    ...
    const searchQuery = currentFilters.search || currentFilters.searchTerm;
    if (searchQuery) {
