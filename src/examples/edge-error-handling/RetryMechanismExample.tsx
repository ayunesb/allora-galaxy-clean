  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mockData: ExampleData[] = [
        { id: '1', name: 'Example Item 1', status: 'active', timestamp: new Date().toISOString() },
        { id: '2', name: 'Example Item 2', status: 'pending', timestamp: new Date().toISOString() },
        { id: '3', name: 'Example Item 3', status: 'completed', timestamp: new Date().toISOString() }
      ];
    ...
    <div className="space-y-4">
      {items.map(item => (
        <div key={item.id} className="flex justify-between items-center border-b pb-2">
          ...
          <Badge variant={item.status === 'active' ? 'default' : item.status === 'pending' ? 'outline' : 'secondary'}>
            {item.status}
          </Badge>
        </div>
      ))}
      ...
      <CardContent>
        {!isLoading && !data && !error ? (
          <div className="flex flex-col items-center justify-center py-8">
            <p className="text-muted-foreground mb-4">Click the button to fetch data</p>
            <Button onClick={handleInitialFetch}>
              Fetch Data
            </Button>
          </div>
        ) : (
