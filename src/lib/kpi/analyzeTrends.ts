 ...
 */
export const analyzeKpiTrend = (kpi: KPI) => {
 ...
 */
export const groupAndAnalyzeKpis = (kpis: KPI[]) => {
...
export const getLatestKpiValues = (kpis: KPI[]): Map<string, KPI> => {
  ...
  kpisByName.forEach((kpiGroup, name) => {
    const sortedKpis = kpiGroup.sort(
...
export const calculateFinancialHealthScore = (kpis: KPI[]): number => {
  ...
  if (mrr && mrr.previous_value) {
    const mrrPercentChange = calculatePercentChange(
      mrr.value,
      mrr.previous_value,
    );
 ...
 */
export const formatKpiTrendForDisplay = (
  trend: KPITrend,
): {
  displayValue: string;
