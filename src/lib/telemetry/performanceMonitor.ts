    ...
    const fidObserver = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        reportPerformanceMeasurement(
          {
            name: "web-vitals:FID",
            startTime: (entry as any).processingStart ?? entry.startTime,
            endTime: (entry as any).processingEnd ?? entry.duration,
            duration:
              ((entry as any).processingEnd ?? entry.duration) -
              ((entry as any).processingStart ?? 0),
            metadata: {
