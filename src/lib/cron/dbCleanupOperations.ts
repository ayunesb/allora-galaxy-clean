...
export async function cleanupOldLogs() {
  try {
    ...
    if (settingsError) {
      throw new Error(`Failed to fetch log retention settings: ${settingsError.message}`);
    }
    ...
    pluginLogsRetention.setDate(pluginLogsRetention.getDate() - retentionDays.plugin_logs);
    ...
    systemLogsRetention.setDate(systemLogsRetention.getDate() - retentionDays.system_logs);
    ...
    await logSystemEvent(
      ...
      'logs_cleanup',
      {
        ...
        plugin_logs_retention_days: retentionDays.plugin_logs,
        system_logs_retention_days: retentionDays.system_logs
      }
    );
