...
export async function evolvePromptWithFeedback(
  ...
  try {
    ...
    if (performance < 0.3) {
      evolvedPrompt += '- This prompt needs significant revision due to poor performance\n';
    } else if (performance < 0.7) {
      evolvedPrompt += '- This prompt is performing adequately but could benefit from improvements\n';
    } else {
      evolvedPrompt += '- This prompt is performing well but minor optimizations could still help\n';
    }
