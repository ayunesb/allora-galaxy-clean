...
export async function checkEvolutionNeeded(agentId: string): Promise<boolean> {
  try {
    ...
    const negativeComments = feedback.filter(
      (comment) => comment.vote_type === "downvote",
    );
    ...
    const errorRate =
      usageStats.filter((log) => log.status === "failure").length /
      usageStats.length;
    ...
    // Check for low XP
    const totalXP = usageStats.reduce(
      (sum, log) => sum + (log.xp_earned || 0),
      0,
    );
