 ...
 */
export async function calculatePerformance(agentId: string): Promise<AgentPerformanceMetrics> {
  try {
    ...
    const failedExecutions = logs.filter(log => log.status === 'failure').length;
    
    const totalXp = logs.reduce((sum, log) => sum + (log.xp_earned || 0), 0);
    const totalExecutionTime = logs.reduce((sum, log) => sum + (log.execution_time || 0), 0);
