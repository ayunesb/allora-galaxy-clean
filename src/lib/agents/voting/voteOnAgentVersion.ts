...
export async function castVote(
  ...
  try {
    ...
    return {
      ...
      upvotes: 0,
      downvotes: 0,
      error: `Failed to cast vote: ${error instanceof Error ? error.message : 'Unknown error'}`,
    };
  }
}
...
export async function voteOnAgentVersion(
  ...
  try {
    ...
    if (existingVote) {
      ...
      const { error: insertError } = await supabase.from("agent_votes").insert({
    ...
    await logSystemEvent("agent", "info", {
      ...
      vote_type: voteType,
      user_id: userId,
      description: "Agent version voted",
