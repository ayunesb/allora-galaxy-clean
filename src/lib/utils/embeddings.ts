...
export function calculateSimilarity(
  ...
  try {
    if (embeddingA.length !== embeddingB.length) {
      throw new Error(
        `Embedding dimensions don't match: ${embeddingA.length} vs ${embeddingB.length}`,
      );
    }
    ...
    const magnitudeA = Math.sqrt(
      embeddingA.reduce((sum, val) => sum + val * val, 0),
    );
    const magnitudeB = Math.sqrt(
      embeddingB.reduce((sum, val) => sum + val * val, 0),
    );
    ...
    logSystemEvent("system", "error", {
      ...
      error: error.message,
      description: "Error calculating similarity",
    }).catch(console.error);
