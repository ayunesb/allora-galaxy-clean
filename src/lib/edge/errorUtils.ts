 ...
 */
export function logError(error: Error, context: Record<string, any> = {}, requestId?: string): void {
  ...
  return (error: Error | any): Response => {
    ...
    return createErrorResponse(
      error.message || 'An unexpected error occurred',
      {
        status,
        code,
        details: process.env.NODE_ENV === 'production' ? undefined : (error.stack || undefined),
        requestId: id
      }
    );
  };
