...
describe("Analyze Prompt Diff Edge Function", () => {
  ...
  it("should call the edge function with the correct parameters", async () => {
    ...
    expect(result.data.diff_summary).toBe(
      "Added context processing instructions",
    );
    expect(result.data.impact_rationale).toBe(
      "This change will improve response quality",
    );
