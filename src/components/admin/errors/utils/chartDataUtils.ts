...
export const prepareErrorTrendsData = (
  ...
  let startDate =
    ...
    (logs.length > 0
      ? new Date(
          Math.min(...logs.map((log) => new Date(log.created_at).getTime())),
        )
      : new Date());
  ...
  let endDate =
    ...
    (logs.length > 0
      ? new Date(
          Math.max(...logs.map((log) => new Date(log.created_at).getTime())),
        )
      : new Date());
  ...
  const dataPoints: ErrorTrendDataPoint[] = dates.map((date) => {
    ...
    // Count by severity
    const critical = logsForDate.filter(
      (log) => log.severity === "critical",
    ).length;
    const high = logsForDate.filter((log) => log.severity === "high").length;
    const medium = logsForDate.filter(
      (log) => log.severity === "medium",
    ).length;
    const low = logsForDate.filter(
      (log) => log.severity === "low" || !log.severity,
    ).length;

    return {
      date: dateStr,
      count: total,
      total: filteredLogs.filter(
