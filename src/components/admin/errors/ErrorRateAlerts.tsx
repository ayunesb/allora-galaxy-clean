              ...
              <TooltipTrigger asChild>
                <div className="flex items-center gap-1 text-sm">
                  ...
                  <span className="text-amber-500 font-medium">
                    {activeAlert.threshold}+ errors/{activeAlert.timeWindow}
                  </span>
                </div>
              </TooltipTrigger>
              <TooltipContent>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="space-y-1">
                <Label htmlFor={`threshold-${rule.id}`} className="text-xs">
                  Threshold
                </Label>
                <div className="flex items-center gap-2">
                  <Input
              ...
              <div className="space-y-1">
                <Label htmlFor={`timeWindow-${rule.id}`} className="text-xs">
                  Time Window
                </Label>
                <Select
                  value={rule.timeWindow}
                  onValueChange={(value) =>
                    handleUpdateRule(rule.id, "timeWindow", value)
                  }
                >
                      ...
                      onClick={() => {
                        if (isActive) {
                          handleUpdateRule(
                            ...
                            rule.notificationChannels.filter(
                              (c) => c !== channel,
                            ),
                          );
                        } else {
                          handleUpdateRule(rule.id, "notificationChannels", [
      ...
      {alertRules.length > 0 && (
        <div className="mt-6 bg-muted/40 p-3 rounded-md border border-dashed">
          ...
          <p className="text-xs text-muted-foreground">
            When error rates exceed your defined thresholds, notifications will
            be automatically sent to your configured channels. Alert states are
            checked every minute, and notifications are rate-limited to prevent
            alert fatigue.
          </p>
          <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
            ...
            <div className="p-2 bg-background rounded border">
              ...
              <div className="text-muted-foreground">
                Posts to configured workspace
              </div>
            </div>
            <div className="p-2 bg-background rounded border">
              <div className="font-medium mb-1">Webhook</div>
              <div className="text-muted-foreground">
                Triggers custom integrations
              </div>
            </div>
            <div className="p-2 bg-background rounded border">
