...
export const LogsExecutionTab: React.FC<LogsExecutionTabProps> = ({
  executionData,
}) => {
  const executionStats = React.useMemo(() => {
    ...
    const totalXp = executionData.reduce(
      (sum, data) => sum + data.xp_earned,
      0,
    );
    const totalTime = executionData.reduce(
      (sum, data) => sum + data.execution_time,
      0,
    );
