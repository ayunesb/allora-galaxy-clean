  ...
  return (
    <Card className="border shadow-sm hover:shadow-md transition-shadow">
      <CardHeader className="pb-2 flex flex-row justify-between items-start">
        <div>
          <CardTitle className="text-lg font-semibold line-clamp-1">{strategy.title}</CardTitle>
          <div className="flex items-center space-x-2 mt-1 text-sm text-muted-foreground">
            <span>Created {formatDate(strategy.created_at)}</span>
          </div>
        </div>
        {strategy.priority && (
          <Badge className={getPriorityColor(strategy.priority)} variant="outline">
            {strategy.priority}
          </Badge>
        )}
      </CardHeader>
      ...
      <CardFooter className="flex justify-between pt-2 border-t">
        <div className="flex items-center text-sm text-muted-foreground">
          ...
          <span>{strategy.due_date ? `Due ${formatDate(strategy.due_date)}` : 'No deadline'}</span>
        </div>
