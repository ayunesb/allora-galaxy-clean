...
export function useStrategyEvolution(
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        historyData?.forEach(
          (item) => item.changed_by && userIds.add(item.changed_by),
        );
        logsData?.forEach(
          (item) => item.executed_by && userIds.add(item.executed_by),
        );
        ...
        setError(
          err instanceof Error
            ? err
            : new Error("Failed to fetch strategy data"),
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const refetch = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to refetch data"),
      );
    } finally {
      setIsLoading(false);
    }
