 ...
 */
export async function fetchUserRole(tenantId: string, userId: string): Promise<string | null> {
  try {
 ...
 */
export async function createTenant(name: string, userId: string, slug?: string): Promise<any> {
  try {
    ...
    const { data: tenant, error: tenantError } = await supabase
      .from('tenants')
      .insert([
        { name, slug: tenantSlug, owner_id: userId }
      ])
      .select()
      .single();
